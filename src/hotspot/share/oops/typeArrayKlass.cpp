...
typeArrayOop TypeArrayKlass::allocate_native_common(int length, bool do_zero, TRAPS) {
  assert(log2_element_size() >= 0, "bad scale");
  check_array_allocation_length(length, max_length(), CHECK_NULL);
  size_t size = typeArrayOopDesc::object_size(layout_helper(), length);

  NatArrayAllocator allocator(this, size, length, CHECK_NULL);
  return (typeArrayOop) allocator.allocateNative();
}
