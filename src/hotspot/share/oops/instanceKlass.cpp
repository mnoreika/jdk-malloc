...
objArrayOop InstanceKlass::allocate_native_objArray(int length, TRAPS) {
  check_array_allocation_length(length, arrayOopDesc::max_array_length(T_OBJECT), CHECK_NULL);
  int size = objArrayOopDesc::object_size(length);
  Klass* ak = array_klass(1, CHECK_NULL);
  NatArrayAllocator allocator(ak, size, length, CHECK_NULL);
  return (objArrayOop) allocator.allocateNative();
}

instanceOop InstanceKlass::allocate_native_instance(TRAPS) {
  int size = size_helper();
  NatAllocator allocator(this, size, CHECK_NULL);
  return (instanceOop) allocator.allocateNative();
}
