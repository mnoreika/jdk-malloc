...
JVM_ENTRY(jobject, JVM_Allocate(JNIEnv *env, jclass clazz))
  JVMWrapper("JVM_Allocate");
  ...
  instanceOop ip = InstanceKlass::cast(k)->allocate_native_instance(THREAD);

  jobject ret = NULL;
  ret = JNIHandles::make_local(env, ip);

  return ret;
JVM_END

JVM_ENTRY(jobjectArray, JVM_AllocateObjectArray(JNIEnv *env, jclass clazz, jint length))
  JVMWrapper("JVM_AllocateObjectArray");
  ...
  objArrayOop p = InstanceKlass::cast(k)->allocate_native_objArray(length, THREAD);
  objArrayHandle result (THREAD, p);

  return (jobjectArray) JNIHandles::make_local(env, result());
JVM_END

JVM_ENTRY(jarray, JVM_AllocatePrimitiveArray(JNIEnv *env, jint type_code, jint length))
  JVMWrapper("JVM_AllocatePrimitiveArray");

  // Convert type code to basic type
  BasicType type = (BasicType) type_code;

  Klass* type_asKlassOop = Universe::typeArrayKlassObj(type);
  TypeArrayKlass* type_asArrayKlass = TypeArrayKlass::cast(type_asKlassOop);

  typeArrayOop result = type_asArrayKlass->allocate_native_common(length, false, THREAD);

  return (jarray) JNIHandles::make_local(env, result);
JVM_END

JVM_ENTRY_NO_ENV(void, JVM_Free(jobject obj))
  JVMWrapper("JVM_Free");
  ...
  oop p = JNIHandles::resolve_non_null(obj);

  NativeAllocator na = NativeAllocator();
  na.free(p);
JVM_END

JVM_ENTRY_NO_ENV(void, JVM_FreeObjectArray(jobjectArray objArray))
  JVMWrapper("JVM_FreeObjectArray");
  ...
  oop p = JNIHandles::resolve_non_null(objArray);
