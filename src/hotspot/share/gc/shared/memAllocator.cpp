...
oop NatAllocator::initialize(HeapWord* mem) const {
  assert(mem != NULL, "cannot initialize NULL object");
  const size_t hs = oopDesc::header_size();
  assert(_word_size >= hs, "unexpected object size");
  oopDesc::set_klass_gap(mem, 0);
  ...
  // log_info(gc)("Initialised allocated memory.");

  return finish(mem);
}

oop NatAllocator::allocateNative() {
  oop obj = NULL;

  {
    HeapWord* mem = (HeapWord*) std::malloc(_word_size * BytesPerWord);
    // log_info(gc)("Malloced memory for new object.");
...
oop NatArrayAllocator::initialize(HeapWord* mem) const {
...
oop NatArrayAllocator::allocateNative() {
  oop obj = NULL;
