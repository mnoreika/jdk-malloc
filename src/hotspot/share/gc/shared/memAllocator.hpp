...
class NatAllocator: public ObjAllocator {
public:
  NatAllocator(Klass* klass, size_t word_size, Thread* thread = Thread::current())
    : ObjAllocator(klass, word_size, thread) {}
  virtual oop initialize(HeapWord* mem) const;
  oop allocateNative();
};

class NatArrayAllocator: public MemAllocator {
  const int _length;
public:
  NatArrayAllocator(Klass* klass, size_t word_size, int length, Thread* thread = Thread::current())
    : MemAllocator(klass, word_size, thread),
    _length(length) {}
  virtual oop initialize(HeapWord* mem) const;
  oop allocateNative();
